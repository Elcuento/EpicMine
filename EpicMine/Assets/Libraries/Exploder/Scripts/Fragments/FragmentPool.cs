...
namespace Exploder
{
    ...
    public class FragmentPool : MonoBehaviour
    {
        ...
        public void DestroyFragments(Fragment[] fragmentList)
        {
            var newList = pool.ToList();
            for (var i = 0; i < fragmentList.Length; i++)
            {
                if (newList.Contains(fragmentList[i]))
                {
                    Destroy(pool[i].gameObject);
                    newList.Remove(pool[i]);
                }
            }
        }
        public Fragment[] GetAllCrackedFragmentsByObject(CrackedObject obj)
        {
            if(pool==null) return new Fragment[0];

            var crackedList = new List<Fragment>();
            foreach (var frag in pool)
            {
                if (frag.CrackedOwner == obj)
                {
                    crackedList.Add(frag);
                }
            }

            return crackedList.ToArray();
        }
