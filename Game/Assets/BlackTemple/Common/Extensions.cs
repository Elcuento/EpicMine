...
namespace BlackTemple.Common
{
    public static class Extensions
    {
        ...
        public static void ClearChildObjects(this Transform transform, List<GameObject> ignore)
        {
        ...
        public static bool CheckText(string nick)
        {
            foreach (var symbol in nick)
            {
                if (!Regex.IsMatch(symbol.ToString(), "[A-Za-z\\d_]"))
                {
                    return false;
                };
            }
        ...
        public static void Fade(this GameObject obj, float to, float speed, float delay = 0)
        {
            var childsI = obj.GetComponentsInChildren<Image>();
            var childsR = obj.GetComponentsInChildren<SpriteRenderer>();
            var childsT = obj.GetComponentsInChildren<TextMeshProUGUI>();

            foreach (var child in childsI)
            {
                if(child.tag != "IgnoreFade")
                child.DOFade(to, speed).SetDelay(delay);
            }
        ...
        public static Dictionary<T, TU> GetDictionaryBySplitKeyValuePair<T, TU>(string str, char splitter = ',')
        {
            ...
            foreach (var pairStr in array)
            {
                ...
                {
                    Debug.Log("error " + pair.Key + ":" + pair.Value);
                    continue;
                }
