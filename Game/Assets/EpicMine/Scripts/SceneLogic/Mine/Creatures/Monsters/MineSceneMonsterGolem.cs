...
public class MineSceneMonsterGolem : MineSceneMonster
{
    [SerializeField] private Animator _eyeAnimation;
    [SerializeField] private MonsterGolemDataReferences _data;

    [SerializeField] private GameObject _abilityParticle;

    [SerializeField] private List<MeshRenderer> _deathRenders;
    ...
    protected override void OnReadyUseAbility()
    {
        ...
        if (_castingAbility.Type == MonsterAbilityType.OtherApply)
        {
            ...
            _animation.Play("castAbilityStart");
        }
        else
        {
            _abilityParticle.SetActive(true);
    ...
    public void OnGetDeathCallback()
    {
        _meshRender.enabled = false;
        foreach (var meshRenderer in _deathRenders)
        {
