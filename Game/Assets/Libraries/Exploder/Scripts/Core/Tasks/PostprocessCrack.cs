    ...
    class PostprocessCrack : Postprocess
    {
        ...
        public override bool Run(float frameBudget)
        {
            ...
            while (core.poolIdx < count)
            {
                ...
                if (mesh.option && mesh.option.FragmentMaterials!=null && mesh.option.FragmentMaterials.Length > 0)
                {
                    fragment.meshRenderer.sharedMaterials = mesh.option.FragmentMaterials;
                }
                else
                {
                    if (core.parameters.FragmentOptions.FragmentMaterials != null)
                    {
                        fragment.meshRenderer.sharedMaterials = core.parameters.FragmentOptions.FragmentMaterials;
                    }
                    else
                    {
                        fragment.meshRenderer.sharedMaterials = mesh.materials;
                    }
                }
                ...
                fragment.CrackedOwner = crackedObject;
